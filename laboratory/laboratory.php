    ...
    function login()
    {
        ...
        if ($stmt->num_rows > 0) {
            ...
            while ($row = $stmt->fetch_assoc()) {
                $arr = array('status' => 200, 'message' => 'User login successfuly', 'details' => array('name' => $row["name"], 'eamil' => $row["email"],"id"=>$row["id"]));
    ...
    function register()
    {
        if (!$this->checkemail()) {
            ...
            if ($this->conn->query($sql) === TRUE) {
                $arr = array('status' => 200, 'message' => 'User added successfuly', 'details' => array('name' => $this->pharmacy_name, 'eamil' => $this->pharmacy_email,"id"=>$this->conn->insert_id));
                echo json_encode($arr);
            } else {
